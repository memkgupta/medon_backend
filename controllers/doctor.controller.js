...
export const requestForRegisteringDoctor = async(req,res,next)=>{
    ...
    const {name,qualifications,gender,timeSlots,licenseNumber,contactInfo,consultationFee,operationalWeek,speciality} = req.body;
...
try {
    const doctor = await Doctor.create({
        name:name,userId:user._id,qualifications:JSON.parse(qualifications),
        gender:gender,
    ...
    res.status(200).json({success:true,message:"Request Sent successfully Our helper visit on your given address",data:{id:doctor._id}});
} catch (error) {
    console.log(error);
    return next(new ErrorHandler(error.message,500));
}
...
doctor.accountDetails({accountName:accountName,accountNo:accountNo,ifscCode:ifscCode});
    await doctor.save();
...
export const requestStatus = async(req,res,next)=>{
    ...
    try {
        ...
        res.status(200).json({success:true,request:{id:request._id,isVerified:request.isVerified}});
    } catch (error) {
...
export const findDoctors = async(req,res,next)=>{
    ...
    try {
        const totalDoctors = await Doctor.aggregate([...pipeline,{
            $count:"totalResults"
        }])
        const doctors = await Doctor.aggregate(pipeline).limit(limit).skip((page-1)*limit);
        
        res.status(200).json({page:page,totalDoctors:totalDoctors.totalResults,doctors});
    } catch (error) {
...
console.log(req.user);
    ...
    try {
        const bookings = await Booking.find({doctor:doctor._id,isAttended:false}).sort({date:1});
